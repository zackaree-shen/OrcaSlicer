...
def color_lithophane_engine(rgb_image, mode=LithoMode.LAYERED, order=ColorOrder.CMY,
                            ...
                            dtop_quantize_step=0.0,
                            dtop_cmy_cover_margin=0.0):
    ...
    if _p1:
        ...
        # Iter 50e: morphological open-close in the P1 path too. It was only
        # applied to non-P1 modes, leaving P1's merged-but-still-spiky dTop
        # surfaces with jagged edges / comb-like spikes (user screenshot).
        # detail_level drives radius: 1.0 -> radius 0 (no-op, baseline-safe),
        # 0.3 -> radius 2 (strong spike suppression). Apply BEFORE CMY re-solve
        # so the smoother dTop constrains CMY correctly.
        if morph_radius > 0:
        ...
        # (c3b) Iteration 50e: ensure the front white cap is always taller than
        # the CMY layers. In chroma_decouple mode CMY carries pure colour and
        # the white relief owns luminance, but if dTop < max(dC,dM,dY) the CMY
        # layers physically protrude above the white cap in slicer view. Force
        # dTop >= max(CMY thickness) + margin everywhere. Applied AFTER
        # recalibration/luminance match so it only overrides the cap geometry,
        # not the colour balance.
        if dtop_cmy_cover_margin is not None and float(dtop_cmy_cover_margin) > 0.0:
            cmy_top = np.maximum(np.maximum(dC, dM), dY)
            dTop = np.maximum(dTop, cmy_top + float(dtop_cmy_cover_margin))
            dTop = np.clip(dTop, 0.0, float(top_max))
        # (c4) Iteration 50d: quantize dTop to discrete layer-height-aligned
        # terraces. This is what finally makes the slicer fill the white layer
        # with solid infill instead of tracing porous micro-contours around
        # every tiny height variation. Applied AFTER all merging/recalibration
        # so it surfaces the already-clean plateaus.
        if dtop_quantize_step and float(dtop_quantize_step) > 0.0:
            from litho_color import quantize_dtop
            dTop = quantize_dtop(dTop, step=float(dtop_quantize_step),
                                 top_max=float(top_max))
    ...
    # (c4) Iteration 50d: quantize dTop to discrete layer-height-aligned
    # terraces so the slicer fills the white layer with solid infill. Applies
    # to all modes (P1 already quantizes earlier so honest dE reflects it).
    if dtop_quantize_step and float(dtop_quantize_step) > 0.0:
        from litho_color import quantize_dtop
        dTop = quantize_dtop(dTop, step=float(dtop_quantize_step),
                             top_max=float(top_max))
