...
def quantize_dtop(dTop, step=0.0, top_max=2.0):
    """Quantize dTop into discrete terraces of height `step`.

    Iteration 50d: the slicer generates porous, fragmented paths when the
    white-relief surface has tiny continuous height variations (it tries to
    trace contours at every micro-step). Rounding dTop to a small number of
    layer-height-aligned levels turns many micro-patches into one large flat
    plateau, so the slicer can fill it with continuous infill instead of
    disjoint contour fragments. step=0 disables (returns input unchanged).

    Typical step: 0.08-0.15 mm, i.e. one or two layer heights. The output is
    clipped to [0, top_max]."""
    if step is None or float(step) <= 0.0:
        return dTop
    step = float(step)
    return np.clip(np.round(np.asarray(dTop, dtype=np.float64) / step) * step,
                   0.0, float(top_max))
...
def merge_features(rgb_uint8, dTop_in, top_max=2.0, lum_tol=0.05,
                   ...
                   dtop_median_size=0, highlight_protect=0.0):
    ...
    highlight_protect : float in [0,1]. If > 0, pixels whose linear luminance
        exceeds the local (gaussian sigma=3) mean by more than this threshold
        are marked as "highlights". Any connected component containing at least
        one highlight pixel is EXEMPT from min_size absorption, preserving
        small bright spots (coins, reflections, speculars, light text) that
        would otherwise be merged into darker neighbours and lost. 0 disables.
        Typical 0.05-0.10 (linear luminance). The mask is computed on the
        ORIGINAL image, so highlights survive even after aggressive merging.
    """
    if strength <= 0.0:
        return dTop_in
    ...
    # Iter 50c: protect small bright highlights from min_size absorption.
    # A "highlight" is a pixel significantly brighter than its local
    # neighbourhood (gaussian-smoothed mean). Any component containing such a
    # pixel is kept, even if smaller than min_size, preserving coins,
    # reflections, light text, and other specular-like details that aggressive
    # plateau merging would otherwise flatten into the surrounding dark paper.
    protected_labels = None
    if min_size > 0 and highlight_protect > 0.0:
        local_mean = gaussian_filter(L, sigma=3.0)
        highlight_mask = (L - local_mean) > float(highlight_protect)
        if highlight_mask.any():
            protected_labels = np.unique(labels[highlight_mask.ravel()])
    ...
    if min_size > 0:
        ...
        for _ in range(5):
            counts = np.bincount(labels, minlength=int(labels.max()) + 1)
            small_mask_full = counts < min_size
            if protected_labels is not None and protected_labels.size:
                small_mask_full[protected_labels] = False
            if not small_mask_full.any():
                break
            boundary = cl_full != cr_full
            if not boundary.any():
